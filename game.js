...
// === Sounds (Base64) ===
const shootSound = new Audio("data:audio/wav;base64,UklGRiQAAABXQVZFZm10IBAAAAABAAEAESsAACJWAAACABAAZGF0YQAAAAA=");
const hitSound = new Audio("data:audio/wav;base64,UklGRiQAAABXQVZFZm10IBAAAAABAAEAESsAACJWAAACABAAZGF0YQAAAAA=");
const bossHitSound = new Audio("data:audio/wav;base64,UklGRiQAAABXQVZFZm10IBAAAAABAAEAESsAACJWAAACABAAZGF0YQAAAAA=");
...
ectx.beginPath(); ectx.arc(20, 25, 6, 0, Math.PI*2); ectx.fill();
...
function gameLoop(){
    ...
    enemies.forEach((enemy,ei)=>{
        ...
        if(enemy.x<0 && !enemy.isBoss){ enemies.splice(ei,1); lives--; if(lives<=0) endGame(); }
        ...
        if(detectCollision(player,enemy)){ lives--; enemies.splice(ei,1); if(lives<=0) endGame(); }
    });
