...
ectx.beginPath();
ectx.arc(20, 25, 6, 0, Math.PI*2);
ectx.fill();
ectx.beginPath();
ectx.arc(40, 25, 6, 0, Math.PI*2);
ectx.fill();
...
class Bullet {
    constructor(x,y){ this.x=x; this.y=y; this.width=10; this.height=4; this.speed=8; }
    draw(){ ctx.fillStyle="#00ffff"; ctx.fillRect(this.x,this.y,this.width,this.height);}
...
function gameLoop(){
    ...
    enemies.forEach((enemy,ei)=>{
        ...
        if(enemy.x<0){ enemies.splice(ei,1); lives--; }
...
document.addEventListener("keydown", e=>{ keys[e.key]=true; if(e.key===" ") bullets.push(new Bullet(player.x+player.width, player.y+player.height/2)); });
